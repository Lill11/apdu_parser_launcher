...
class ApduTableModel(QAbstractTableModel):
    headers = ["Index", "Command", "Response", "Category", "Description"]
...
class ApduFilterProxyModel(QSortFilterProxyModel):
    ...
    def filterAcceptsRow(self, source_row: int, source_parent: QModelIndex) -> bool:
        ...
        if self.filter_mode != FilterMode.ALL and self.filter_mode.value not in row.filters:
            return False
        if self.search_text:
            haystack = " ".join([row.command, row.response, row.category, row.description, row.note]).lower()
...
class ResultTabs(QWidget):
    def __init__(self) -> None:
        ...
        self.apdu_view.horizontalHeader().setSectionResizeMode(4, QHeaderView.ResizeMode.Stretch)
        self.apdu_view.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.Stretch)
    ...
    def set_result(self, item_name: str, result: ParseResult | None) -> None:
        ...
        self.apdu_model.set_rows(result.apdus)
        self._render_analysis()
    ...
    def _render_analysis(self) -> None:
        ...
        for event in self.current_result.analysis:
            ...
            if not include and event.event_type == "reset":
                include = filter_mode == FilterMode.ES10
            ...
            if not include:
                continue
            parts.append(
                f"<div class='analysisEvent'><div class='analysisTitle'>[{event.index:04d}] {event.title}</div>"
    ...
    def copy_all(self) -> None:
        ...
        for i in range(self.apdu_proxy.rowCount()):
            ...
            rows.append(f"[{row.index:04d}] {row.command} -> {row.response} | {row.description}")
